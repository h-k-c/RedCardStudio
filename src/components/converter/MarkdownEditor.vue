...
function getCursorPosition(): number {
  ...
  if (!cmContent) return 0
  
  const contentRange = document.createRange()
  contentRange.setStart(cmContent, 0)
  contentRange.setEnd(container, range.startOffset)
  
  return contentRange.toString().length
}

/**
 * 查找光标后最近的标题位置
 */
function findNextTitlePosition(text: string, cursorPos: number): number {
...
function handlePageBreak() {
  const currentText = editorValue.value
  if (!currentText) {
    showToast('内容为空，无法分页')
    return
  }
  ...
  // 查找下一个标题位置
  const titlePos = findNextTitlePosition(currentText, cursorPos)
  
  if (titlePos === -1) {
    showToast('未找到 ## 或 ### 标题，无法分页')
    return
  }
